...
const cliente_slice = createSlice({
    ...
    reducers: {
        getCliente: (state: ClienteState, action) => {
            if (action.payload !== null) state.cliente = action.payload;
        },
        getListaClientes: (state: ClienteState, action) => {
            if (action.payload !== null) state.lista_clientes = action.payload;
        }
    }
})
